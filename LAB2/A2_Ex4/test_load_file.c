...
int main(int argc, char const *argv[])
{
    ...
    int *A = load_file("test-10-1-problem");

    int num_elements = A[0];
    /* apply sorting on A
    note: A[0] is the size (num_elements) */

    quickSort(A + 1, 0, num_elements - 1);
    printf("QuickSort: ");
    ...
    /* load "solution" example file (same directory)*/
    int *B = load_file("test-10-1-solution");
    printf("solution: ");
    for (int i = 1; i <= num_elements; i++)
    {
        printf("%d ", B[i]);
    ...
    printf("\n");
    /* compare [now sorted] A with [already sorted] B */
